...
OpsRAG is a compact technical knowledge assistant built incrementally as a production-oriented RAG portfolio project. The current implementation loads local documents, creates deterministic chunks, indexes normalized BGE embeddings in Qdrant, fuses dense and BM25 retrieval with RRF, reranks the hybrid candidates with a cross-encoder, and generates an answer with verified source attribution through a provider-neutral LLM interface.

## Current scope

Completed through **Milestone 8 — Source Attribution**:
